  ...
  if ($("body").is(".upload-php, .post-type-attachment")) {
    ...
    let buildList = function (obj) {
        ...
        $.each(obj, function (key, value) {
          ...
          /** check extension */
          if (
            value[vkey].file_ext === "jpg" ||
            value[vkey].file_ext === "jpeg" ||
            value[vkey].file_ext === "svg"
          ) {
          ...
          list +=
            ...
            '<input class="KiTT-media-result-checkbox" type="checkbox" data-file_path="' +
            value[vkey].file_src +
            '" data-file_id="' +
            id +
            '">' +
            ...
            "</div>" +
            "<span>Size: " +
            value[vkey].file_size[1] +
            ' | <strong style="color:' +
            (vkey === "reference" ? "#3ab829" : "#E63939") +
            '">' +
            vkey +
            "</strong></span>" +
            "</div>" +
  ...
  if ($("body").is(".post-php, .post-new-php")) {
    ...
    $("#KiTT-meta-box-homepage").on("change", function () {
      ...
      if (this.value !== "0") {
        $.ajax({
          ...
          success: function (data, textStatus, XMLHttpRequest) {
            $("#KiTT-meta-box-homepage-msg").html(
              '<span style="color: #3ab829;">success:</span> ' +
                title +
                " is the new Homepage!"
            );
          },
          error: function (XMLHttpRequest, textStatus, error) {
            $("#KiTT-meta-box-homepage-msg").html(
              '<span style="color: #E63939;">error:</span> There went something wrong!'
            );
            console.error(error);
          }
        });
    ...
    $("#KiTT-meta-box-error-page").on("change", function () {
      ...
      if (this.value !== "0") {
        $.ajax({
          ...
          success: function (data, textStatus, XMLHttpRequest) {
            $("#KiTT-meta-box-error-page-msg").html(
              '<span style="color: #3ab829;">success:</span> ' +
                title +
                " is the new Error Page!"
            );
          },
          error: function (XMLHttpRequest, textStatus, error) {
            $("#KiTT-meta-box-error-page-msg").html(
    ...
    $("#publish, #save-post").one("click", function () {
      let id = $("#KiTT-meta_box_seo table:first-child").data("post-id"),
        ...
        keywords = $(
          "#KiTT-meta_box_seo table tr td:first-child textarea"
        ).val(),
        description = $(
          "#KiTT-meta_box_seo table tr td:last-child textarea"
        ).val(),
        date = new Date(
          $("#KiTT-meta_box_seo #KiTT-robots-unavailable_after").val()
        ),
        ...
        canonical = $("#KiTT-meta_box_seo #KiTT-canonical-links")
          .val()
          .replace(/[\s\n]+/g, "")
          .split(",")
          .filter(val => {
      ...
      if (String(date) !== "Invalid Date") {
        ...
        unavailableAfter =
          ...
          "23:59:59 " +
          universalTime +
          (timezoneOffset > 9 ? timezoneOffset : "0" + timezoneOffset);
      }

      let robots = [
        $("#KiTT-meta_box_seo #KiTT-robots-index").prop("checked")
          ? "index"
          : "noindex",
        $("#KiTT-meta_box_seo #KiTT-robots-follow").prop("checked")
          ? "follow"
          : "nofollow",
        $("#KiTT-meta_box_seo #KiTT-robots-noimageindex").prop("checked")
          ? "noimageindex"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-noarchive").prop("checked")
          ? "noarchive"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-nocache").prop("checked")
          ? "nocache"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-nosnippet").prop("checked")
          ? "nosnippet"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-nositelinkssearchbox").prop(
          "checked"
        )
          ? "nositelinkssearchbox"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-nopagereadaloud").prop("checked")
          ? "nopagereadaloud"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-noyaca").prop("checked")
          ? "noyaca"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-notranslate").prop("checked")
          ? "notranslate"
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-max_snippet").val() >= -1
          ? "max-snippet:" +
            $("#KiTT-meta_box_seo #KiTT-robots-max_snippet").val()
          : null,
        $("#KiTT-meta_box_seo #KiTT-robots-max_video_preview").val() >= -1
          ? "max-video-preview:" +
            $("#KiTT-meta_box_seo #KiTT-robots-max_video_preview").val()
          : null,
        "max-image-preview:" +
          $("#KiTT-meta_box_seo #KiTT-robots-max_image_preview").val(),
        unavailableAfter
      ]
        .filter(function (el) {
          return el !== null;
